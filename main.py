...
# Liste des fichiers DB à sauvegarder
DB_FILES = ["cash_bet4_secure.db", "channels_config.db"]
BACKUP_CHANNEL_ID = -1003239143402   # 👉 Mets ici ton vrai canal privé backup


async def send_backup_zip(bot, manual=False):
    """Crée un ZIP des fichiers DB + envoie dans le canal backup."""
    ...
    with zipfile.ZipFile(zip_name, "w", zipfile.ZIP_DEFLATED) as zipf:
        for db in DB_FILES:
            if os.path.exists(db):
                ...
                print(f"⚠️ Fichier introuvable : {db}")

    # 📤 Envoi vers le canal backup
    caption = (
        f"📦 <b>Backup {'manuel' if manual else 'automatique'}</b>\n"
        f"🕒 {now}\n"
        f"📁 Fichiers : {', '.join(DB_FILES)}"
    )
...
async def get_missing_channels_for_user(bot, user_id: str) -> list[str]:
    ...
    for c in channels:
        # 🚫 Canal sans lien = canal inactif → ne pas vérifier
        if not c["url"]:
            continue
...
async def check_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for c in channels:
        # 🚫 Si aucun lien n’est configuré pour ce canal → on ignore ce canal
        if not c["url"]:
            continue

        # Vérification normale d’abonnement
        chat = f"@{c['username']}"
        try:
...
async def admi_menu_callback_from_message(chat_id: str, bot, context):
    buttons = [
        ...
        [InlineKeyboardButton("📢 Publier faux bonus 1XBET/MELBET", callback_data="admi_fake_bonus")],[InlineKeyboardButton("🌀 Générer code mystère", callback_data="admi_generate_code")],[InlineKeyboardButton("📦 Backup maintenant", callback_data="admi_backup_now")],  # NOUVEAU
...
async def support_withdraw_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =========================
# Code magique de fiabilisation Cash Bet4 🔮
# =========================
...
async def magic_channels_sanity_check():
    ...
    async with aiosqlite.connect(CHANNELS_DB) as db:
        ...
        # 2️⃣  Corriger les enregistrements mal formés
        ...
        for cid, url, username in rows:
            ...
            # Corriger le pseudo s’il est vide
            ...
            # Corriger les liens privés mal placés
    ...
    print("✅ Vérification magique terminée — base prête à l’emploi !")
    # ============================
# 📦 CALLBACK : Backup manuel ADMI
# ============================
...
async def admi_backup_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        "<b>📦 Backup manuel effectué et envoyé dans le canal.</b>",
        parse_mode=ParseMode.HTML
    )
 # ------------------------------
...
async def main():
    ...
    await magic_channels_sanity_check()     # 🔮 Auto-réparation des canaux
    await init_channels_db()                # ✅ DB canaux
    await init_db()                         # ✅ DB principale
    ...
    # === GESTION DES CANAUX (public + privé) ===
    app.add_handler(CallbackQueryHandler(
    ...
    print("🤖 Cash_Bet4 totalement opérationnel (sécurité + backups + anti-fraude + retraits + gestion canaux)")
    ...
    while True:
        try:
            ...
            print(f"⚠️ Déconnexion détectée : {e}")
            print("⏳ Nouvelle tentative dans 5 secondes...")
            time.sleep(5)


# ============================================================
# 🚀 LANCEMENT AUTOMATIQUE DU BOT SUR RENDER
# ============================================================
if __name__ == "__main__":
    ...
    print("🚀 Démarrage du bot Cash Bet4...")
    try:
